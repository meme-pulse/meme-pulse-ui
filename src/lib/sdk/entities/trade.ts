...
import { ONE, TradeType } from '../constants/const';
import { ChainId } from '../constants/const';

import JSBI from 'jsbi';

import { RouteV2 } from './route';
import { LB_QUOTER_V22_ADDRESS, LB_ROUTER_V22_ADDRESS, WNATIVE } from '../constants/addresses';
import { toHex, validateAndParseAddress, isZero, ZERO_HEX } from '../hex';
import {
  type TradeOptions,
  type TradeOptionsDeadline,
  type TradeFee,
  type SwapParameters,
  type Quote,
  type RouterPathParameters,
} from '../types';

import { LBQuoterV22ABI, LBRouterV22ABI } from '../constants/abis/ts';
...
export class TradeV2 {
  ...
  public constructor(
    ...
    const outputAmount = new TokenAmount(tokenOut, JSBI.BigInt(quote.amounts[quote.amounts.length - 1].toString()));
  ...
  public swapCallParameters(options: TradeOptions | TradeOptionsDeadline): SwapParameters {
    ...
    const binSteps: string[] = this.quote.binSteps.map((bin) => '0x' + bin.toString(16));
  ...
  public async getTradeFee(): Promise<TradeFee> {
    ...
    const feesPct = this.quote.fees.map((bn) => new Percent(JSBI.BigInt(bn.toString()), JSBI.BigInt(1e18)));
    ...
    const feesTokenIn = fees.map((fee, i) => {
      ...
      const midPrice = new Fraction(JSBI.BigInt(amounts[0].toString()), JSBI.BigInt(amounts[i].toString()));
      return midPrice.multiply(fee).quotient;
    });

    // sum of all fees; e.g. 0.0051 WAVAX
    const totalFee = feesTokenIn.reduce((a, b) => JSBI.add(a, b), JSBI.BigInt('0'));
   ...
   */
  public async estimateGas(publicClient: PublicClient, account: Hex, chainId: ChainId, slippageTolerance: Percent): Promise<bigint> {
    ...
    // const useV22 = true;
    const routerAddress = LB_ROUTER_V22_ADDRESS[chainId];
    const abi = LBRouterV22ABI;
  ...
  public static async getTradesExactIn(
    ...
    try {
      ...
      const quoterAddress = LB_QUOTER_V22_ADDRESS[chainId];

      const abi = LBQuoterV22ABI;
  ...
  public static async getTradesExactOut(
    ...
    // handle wavax<->avax wrap swaps
    const isWrapSwap =
      (isNativeIn && tokenAmountOut.token.address === WNATIVE[chainId].address) ||
   ...
   * @returns {TradeV2}
   */
  public static chooseBestTrade(trades: TradeV2[], isExactIn: boolean): TradeV2 | undefined {
    if (trades.length === 0) {
      return undefined;
    }
    ...
    const bestTrade = tradesWithGas.reduce((previousTrade, currentTrade) =>
      tradeType === TradeType.EXACT_INPUT
        ? currentTrade.swapOutcome.greaterThan(previousTrade.swapOutcome)
          ? currentTrade
          : previousTrade
        : currentTrade.trade.inputAmount.greaterThan('0') && currentTrade.swapOutcome.lessThan(previousTrade.swapOutcome)
        ? currentTrade
        : previousTrade
    );
