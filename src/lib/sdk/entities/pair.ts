...
import { ONE } from '../constants/const';

import { type LBPair, type LBPairReservesAndId, type LBPairFeeParameters, type LiquidityDistribution, type BinReserves } from '../types';
import { LB_FACTORY_V22_ADDRESS } from '../constants/addresses';
import { Bin } from './bin';
import { getLiquidityConfig } from '../liquidityDistribution';

import { type Hex, type PublicClient, getAddress } from 'viem';
import { LBFactoryV21ABI } from '../constants/abis/ts/LBFactoryV21';
import { LBPairV21ABI } from '../constants/abis/ts';
...
export class PairV2 {
  ...
  public async fetchAvailableLBPairs(
    ...
    const args = [getAddress(this.token0.address), getAddress(this.token1.address)] as const;
   ...
   * @returns {Promise<LBPair>}
   */
  public async fetchLBPair(binStep: number, version: 'v2' | 'v21' | 'v22', publicClient: PublicClient, chainId: ChainId): Promise<LBPair> {
    const args = [getAddress(this.token0.address), getAddress(this.token1.address), BigInt(binStep)] as const;
   ...
   */
  public static async getFeeParameters(LBPairAddr: Hex, publicClient: PublicClient): Promise<LBPairFeeParameters> {
    return (await publicClient.readContract({
      abi: LBPairV21ABI,
      address: LBPairAddr,
      functionName: 'getStaticFeeParameters',
    })) as unknown as LBPairFeeParameters;
  }
  ...
  public static calculateAmounts(
    ...
    binIds.forEach((binId, i) => {
      ...
      if (binId <= activeBin) {
        const amountY = JSBI.divide(JSBI.multiply(liquidityAmount, reserveY), totalSupply);
        totalAmountY = JSBI.add(amountY, totalAmountY);
      }

      if (binId >= activeBin) {
        const amountX = JSBI.divide(JSBI.multiply(liquidityAmount, reserveX), totalSupply);
  ...
  public addLiquidityParameters(
    ...
    const idSlippage = Bin.getIdSlippageFromPriceSlippage(_priceSlippage, binStep);
  ...
  public calculateAmountsToRemove(
    ...
    // calculate expected total to remove for X and Y
    const { amountX, amountY } = PairV2.calculateAmounts(userPositionIds, activeBin, bins, totalSupplies, amountsToRemove);
